...
void SaveBMP(const char* fname, const std::vector<geometry::Vec3f>& pixels, int w, int h)
{
  ...
  {
    Pixel px;
    px.r       = (char) (std::max(0.f, std::min(1.f, pixels[i].z))  * 255);
    px.g       = (char) (std::max(0.f, std::min(1.f, pixels[i].y))  * 255);
    px.b       = (char) (std::max(0.f, std::min(1.f, pixels[i].x))  * 255);
    pixels2[i] = px;
  }
